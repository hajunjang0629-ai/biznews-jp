...
「あるATMから別のATMへ行き、まだ紙幣が残っているATMを探している女性がいました。」`,
  ...
  {
    id: "cybersecurity-risks-posed-by-over-the-ai-fbd560d5",
    title: "Cybersecurity risks posed by over-the-air tech in autos has analysts concerned",
    titleJa: "アナリストは自動車の無線技術によってもたらされるサイバーセキュリティのリスクを懸念している",
    summaryJa: "アナリストらは、自動車業界では車両システムの更新に無線技術の利用が増えており、サイバー攻撃を受けやすくなっていると述べ、この分野への介入を増やすよう求めている。OTA テクノロジーは、新しいソフトウェア、ファームウェア、修正プログラム、データをインターネットに接続されたデバイスに配信できるワイヤレス技術です。",
    bodyOriginal: `The automotive industry's increasing use of over-the-air technology to update vehicle systems makes it more susceptible to cyberattacks, analysts say, urging more intervention in the sector.
...
As it becomes more widespread, the RSIS' Lim stressed the importance of taking responsibility for the implementation of the tech. "It is crucial for us to be aware of this technology, and to hold entities and governments accountable for how OTA systems are applied, especially how they run quietly in the background of the technologies we use in our everyday lives."`,
    bodyJa: `アナリストらは、自動車業界では車両システムの更新に無線技術の利用が増えており、サイバー攻撃を受けやすくなっていると述べ、この分野への介入を増やすよう求めている。
OTA テクノロジーは、新しいソフトウェア、ファームウェア、修正プログラム、データをインターネットに接続されたデバイスに配信できるワイヤレス技術です。
テスラは、2012 年に自社のモデル S 車に無線アップデートを導入し始めました。オーストラリア戦略政策研究所のサイバー、テクノロジー、セキュリティのフェロー、ジェイソン ヴァン デル シフ氏によると、これによりこの技術の正常化が促進され、現在では自動車業界の多くの分野に無線アップデートが組み込まれていると述べています。
英国スウォンジー大学システムセキュリティ教授のシラージ・アーメド・シャイク氏はCNBCに対し、「この技術は、定期メンテナンス時にリコールやアップデートが必要だった可能性のある従来の方法に比べ、迅速かつ費用対効果の高い方法で車両のシステムを管理できるため、ますます歓迎されている」と語った。
しかし、自動車業界における OTA テクノロジーの浸透が進むにつれて、特に交通インフラに関して懸念が生じています。
シンガポールのS・ラジャラトナム国際大学院の上級アナリスト、ガブリエル・リム氏はCNBCに対し、その使用は「独特の国家安全保障上の懸念」を表していると語った。
「データプライバシーの懸念とは別に、外国の攻撃者が走行中の車両の制御を妨害する可能性は、ノルウェー、デンマーク、英国などの国々が懸念を表明している」とリム氏は付け加えた。
アメリカン・エンタープライズ協会は5月、外国政府のスパイ活動を制限するには自動車セクターを守ることが重要だと警告した。
同報告書は、「外国スパイの脅威から守るために、米国は追加の安全保障見直しを検討し、自動車に搭載される特定の外国製ハードウェアとソフトウェアに対する制限を導入し、データ収集の開示強化を義務付ける必要がある」としている。
「制御システムへのアクセスがあります」
実際のテストで脆弱性が明らかになったことで懸念が生じています。
昨年末、ノルウェーのバス会社ルーターは 2 台のバスでテストを実施し、1 台には OTA テクノロジーに関連する潜在的なリスクがあることが判明しました。
「ルーマニアのSIMカードを介して、モバイルネットワーク経由でバッテリーと電源の制御システムにアクセスできる。したがって、理論上は、このバスは製造業者によって停止または動作不能にされる可能性がある」と同社は述べた。
その後、ルーター氏の調査を受けて英国とデンマークは独自の調査を行うことになり、英国運輸省はこの問題を調査し、国家サイバーセキュリティセンターと緊密に連携していると述べた。
これらの調査は中国企業Yutong製のバスに対して行われたが、技術の普及が進むにつれ、この問題は1つのメーカーや国を超えて広がっているとシャイク教授は述べた。
同氏は、「OTAを採用している他の分野には、海運や鉄道、航空宇宙（特にドローン）、産業機械、ロボット工学などの他の輸送モードが含まれる」と述べた。
RSISのリム氏は、この技術の普及が進むにつれ、この技術の導入に責任を持つことの重要性を強調した。 「私たちにとって、このテクノロジーを認識し、OTAシステムがどのように適用されるか、特にOTAシステムが私たちが日常生活で使用するテクノロジーの背景でどのように静かに実行されるかについて、企業や政府に責任を負わせることが重要です。」`,
    source: "CNBC",
    sourceUrl: "https://www.cnbc.com/2026/07/18/over-the-air-tech-in-vehicles-poses-cybersecurity-risks.html",
    publishedAt: "2026-07-18T23:01:01+00:00",
    category: "テクノロジー",
    imageUrl: "https://images.unsplash.com/photo-1518770660439-4636190af475?w=800&q=80",
    readTime: 4,
  },
